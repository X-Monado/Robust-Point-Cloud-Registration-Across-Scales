...
def scale_estimation(Sxy, Snoise, interval, flag, bound, scale, random_seed=None):
    """
    Estimate scale from scale vectors using RANSAC with true random sampling.
    ...
    random_seed : int or None
        Random seed for reproducibility. If None, uses non-deterministic mode.
    ...
    # If random_seed is None, use deterministic sequential selection (original behavior)
    ...
    if flag == 1:
        ...
        while iter_num < Niter and np.sum(all_point) > 0:
            ...
            # Random selection: if rng is provided, use true random sampling;
            # otherwise, fall back to deterministic sequential selection (original behavior)
            if rng is not None:
                random_ind = rng.randint(0, all_size_f)
            else:
                random_ind = 0
