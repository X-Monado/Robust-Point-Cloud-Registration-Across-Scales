...
def test_mvs_pair(full_data_path):
    """
    Test a single MVS pair
    """
    ...
    # Handle distances format
    if distances.ndim == 1:
        distances = np.column_stack([distances, distances])
...
def main():
    MATCHES_DIR = '/home/zjy24/DeepLearning/FCGF-master/outputs/mvs_pairs_v2'
    
    all_npz_files = sorted(
        [os.path.join(MATCHES_DIR, f) for f in os.listdir(MATCHES_DIR) if f.endswith('.npz')],
        key=lambda x: int(os.path.basename(x).replace('pair_', '').replace('.npz', ''))
    )
    
    print(f"Found {len(all_npz_files)} pairs to test\n")
    
    if not os.path.exists("log"):
        os.makedirs("log")
    
    result_file = "log/result_mvs_fcgf_v2.csv"
    ...
    for i, full_data_path in enumerate(all_npz_files):
        file_id = os.path.basename(full_data_path)
        print(f"\n{'='*60}")
        print(f"[{i+1}/{len(all_npz_files)}] Testing: {file_id}")
        print('='*60)
        
        try:
            S, R_error, T_error, time_val = test_mvs_pair(full_data_path)
            ...
            if S is not None:
                ...
                with open(result_file, mode='a+', newline='', encoding='utf-8') as outfile:
                    ...
                    writer.writerow([file_id, f"{S:.6f}", f"{R_error:.4f}", f"{T_error:.4f}", f"{time_val:.4f}"])
    ...
    print("SUMMARY")
    print('='*60)
    print(f"Total pairs: {len(all_npz_files)}")
    print(f"Successful: {success_count}")
    print(f"Failed: {len(all_npz_files) - success_count}")
    ...
    if success_count > 0:
        ...
        rot_success_5 = sum(1 for r in R_error_list if r < 5)
        rot_success_10 = sum(1 for r in R_error_list if r < 10)
        ...
        print(f"\nRotation Error < 5 deg: {rot_success_5}/{success_count} ({100*rot_success_5/success_count:.1f}%)")
        print(f"Rotation Error < 10 deg: {rot_success_10}/{success_count} ({100*rot_success_10/success_count:.1f}%)")
        print(f"Translation Error < 0.2m: {trans_success}/{success_count} ({100*trans_success/success_count:.1f}%)")
